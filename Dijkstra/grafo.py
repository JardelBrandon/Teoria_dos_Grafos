...
class Grafo:
    ...
    def __init__(self, N=[], A={}):
        ...
        self.dicionarioDaMatrizDeAdjacenciasComPesos = self.adicionarPesoDasArestas()
    ...
    def adicionarPesoDasArestas(self):
        matrizComPesos = copy.deepcopy(self.dicionarioDaMatrizDeAdjacencias)
        for vertice in matrizComPesos:
            for verticeAdjacente in matrizComPesos[vertice]:
                if matrizComPesos[vertice][verticeAdjacente] > 0:
                    print("Digite o peso que será atribuido a aresta que liga os vertices: ",
                          vertice, "->", verticeAdjacente)
                    peso = abs(float(input()))
                    matrizComPesos[vertice][verticeAdjacente] = (
    ...
    def __str__(self):
        ...
        for vertice in self.dicionarioDaMatrizDeAdjacenciasComPesos:
            grafo_str += vertice
            #grafo_str += "\n"
            for verticesAdjacentes in self.dicionarioDaMatrizDeAdjacenciasComPesos[vertice]:
                        grafo_str += " "
                        grafo_str += str(self.dicionarioDaMatrizDeAdjacenciasComPesos[vertice][verticesAdjacentes])
                        grafo_str += "\t"
            grafo_str += "\n"
